...
use mediumi_mp4::boxes::trex::Trex;
use mediumi_mp4::{
    Mp4Box, demuxer, find_codec_config, handler_fourcc, iter_trafs, iter_traks, track_samples,
    traf_samples,
};
...
fn main() {
    // non fragmented mp4
    {
        ...
        let processor = Processor::from_avcc(&samples, &cfg).expect("failed to parse AVCC samples");

        let (re_sample_bytes, re_cfg) = processor.to_avcc(&cfg).expect("failed to serialize AVCC");
        ...
        assert_eq!(re_sample_bytes, original_concat, "AVCC sample bytes differ");
    ...
    // fragmented mp4
    {
        let init_path = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../mediumi-mp4/examples/data/test_init.m4s"
        );
        let seg_path = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../mediumi-mp4/examples/data/test.m4s"
        );
        ...
        assert_eq!(
            re_sample_bytes, original_concat,
            "fMP4 AVCC sample bytes differ"
        );
